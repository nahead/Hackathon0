...
def test_linkedin_api():
    ...
    print("[TEST] Testing LinkedIn API Connection...")

    # Get credentials from environment
    linkedin_access_token = os.getenv('LINKEDIN_ACCESS_TOKEN')

    if not linkedin_access_token:
        print("[ERROR] ERROR: LINKEDIN_ACCESS_TOKEN environment variable not set")
    ...
    try:
        ...
        print("\n[API] Testing API connection...")
        response = requests.get(url, headers=headers, timeout=10)

        if response.status_code == 200:
            user_data = response.json()
            print(f"[OK] Connected to LinkedIn API")
...
def post_to_linkedin(content, dry_run=True):
    ...
    print("\n[TEST] Testing LinkedIn Posting...")

    if dry_run:
        print("[WARN] DRY RUN MODE - No actual posting")
    ...
    if not linkedin_access_token or not linkedin_person_urn:
        print("[ERROR] ERROR: Missing LinkedIn credentials")
...
def process_approved_posts():
    ...
    print("\n[TEST] Checking for Approved Posts...")

    approved_path = Path("AI_Employee_Vault/Approved")
    if not approved_path.exists():
        print("[WARN] No Approved folder found")
        return []
    ...
    print(f"[INBOX] Found {len(approved_posts)} approved post(s)")
...
if __name__ == "__main__":
    ...
    if posts:
        print(f"\n[SEND] Processing {len(posts)} post(s)...")
        ...
        for i, post in enumerate(posts, 1):
            ...
            if success and not dry_run:
                ...
                print(f"   [OK] Moved to Done: {post['file'].name}")

        print("\n" + "=" * 60)
        print("[OK] POSTING TEST COMPLETE")
        print("=" * 60)
    else:
        print("\n" + "=" * 60)
